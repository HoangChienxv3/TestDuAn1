...
public class HoaDonDao {
    ...
    public List<HoaDon> select() {
        String sql = "SELECT MaHD,MaBan,CONVERT(NVARCHAR,NGAYBAN,105),TenBan,TenTaiKhoan,TongTien,MaSuKien,UuDai,TongTienThanhToan,GhiChu,SoHD\n"
                + "FROM HoaDon";
        return select(sql);
    }
    ...
    public List<HoaDon> TimKiemNgay(String id) {
        String sql = "SELECT MaHD,MaBan,CONVERT(NVARCHAR,NGAYBAN,105),TenBan,TenTaiKhoan,TongTien,MaSuKien,UuDai,TongTienThanhToan,GhiChu,SoHD\n"
                + "FROM HoaDon\n"
                + "where NgayBan=?";
        return select(sql, id);
